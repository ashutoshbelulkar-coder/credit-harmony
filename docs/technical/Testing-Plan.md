...
**Scope:** Source-type-level Data Policy configuration UI (consortium wizard → Data policy step → active source types table → per-source-type Configure drawer) and mandatory governance audit entry creation on Save.

| ID | Scenario | Steps | Expected |
|----|----------|-------|----------|
| DP-001 | Unmask policy section is present (consortium-level) | Open Consortium Wizard → Data policy step | “Unmask policy” section appears above Source Types (Full vs Partial) |
| DP-002 | Full vs Partial selection persists while configuring source types | Choose Partial → open/close Configure drawers | Consortium-level policy remains selected; does not reset on drawer open/close |
| DP-003 | Source types data table renders | Open Data policy step | Active master-schema source types list renders in a table |
| DP-004 | (Removed - Replaced by table defaults) | | |
| DP-005 | Per-source-type drawer opens and lists masked fields | Click Configure on a source type row | Right drawer opens for that source type; shows masked fields with allow-unmask checkboxes and Data provider name |
| DP-006 | Drawer scroll works with long lists | Open Configure for source type with many masked fields | Field list is scrollable; header and Save/Cancel remain accessible |
...
| DP-010 | “At least 1 masked” guard blocks unsafe saves | Check every masked field for a source type → Save | Save blocked with error “At least 1 field must remain masked”; selections retained |
| DP-011 | Save persists source-type-scoped policy only | Configure Source Type A → Save → Configure Source Type B | Source Type A keeps its policy; Source Type B remains independent |
| DP-012 | Draft vs saved behavior | Change selections (don’t save) → close drawer → reopen | Unsaved changes are preserved while the source type draft is dirty; saving clears dirty state |
...
| DP-018 | Table performance | View many source types | The page remains responsive; masked counts load progressively per schema |
