...
- **BV-5** — Transparent lineage and subscriber visibility support regulatory accountability and audit readiness.
...
- Product transparency: viewing the data contract, data lineage (submitters → packets → product → consumers), subscribers, version history, and audit trail.
- Pre-go-live enquiry simulation entry point (configuration handoff — see [Section 4](#4-module-context--interactions)).
...
| CAP-8 | Transparency & Lineage | View data contract, lineage, subscribers, version history, and audit. |
| CAP-9 | Pre-Go-Live Simulation | Hand off a configuration for enquiry simulation. |
...
| PC-FR-203 | The Product Owner shall provide business metadata: business unit, target segment, optional SAP item code, sensitivity (Low/Medium/High), legal conditions, tags, and effective start date. |
...
### 7.8 Transparency & Lineage (CAP-8)

| ID | Requirement |
|----|-------------|
| PC-FR-801 | The product detail shall present an **Overview** of business metadata, sensitivity, coverage, and effective dates. |
...
| PC-FR-803 | The product detail shall present **Lineage** showing the flow: data submitters → data packets → this product version → consumers. |
| PC-FR-804 | From lineage, a user shall be able to inspect the field list contributed by any individual packet. |
...
**Business rules**
- **PC-BR-801** — Lineage data submitters are resolved to named member institutions (INT-1); consumers are the product's subscribing institutions.
- **PC-BR-802** — A field is marked **PII** and its attribute **mode** based on governed field semantics.

### 7.9 Pre-Go-Live Simulation (CAP-9)
...
**Business rules**
- **PC-BR-901** — Simulation must not create a real enquiry footprint against a consumer (see [Section 4](#4-module-context--interactions), INT-5; execution owned by Enquiry Simulation).
...
| ID | Rule | Applies To | Business Message Intent |
|----|------|-----------|--------------------------|
| PC-VAL-01 | Product name required and unique across catalogue | UA-01, UA-02, UA-03 | Prevent ambiguous/duplicate product identity |
| PC-VAL-02 | At least one packet selected | UA-03 | A product must expose data |
| PC-VAL-03 | Justification required | UA-03, UA-06–UA-10 | Every governance/lifecycle action is accountable |
| PC-VAL-04 | Reject requires comment | UA-05 | Rejection reason must be recorded |
| PC-VAL-05 | Submitter ≠ approver | UA-04, UA-14 | Enforce separation of duties |
| PC-VAL-06 | Wind-down window is whole number ≥ 1 day | UA-07 | Consumers get a valid migration period |
| PC-VAL-07 | Duplicate definition blocked unless exception requested | UA-03 | Protect catalogue/monetisation integrity |
| PC-VAL-08 | Transition must be permitted by state matrix | UA-06–UA-10 | Prevent invalid lifecycle jumps |
| PC-VAL-09 | Active-version ceiling of 3 enforced | UA-06 | Bound concurrent live definitions |
| PC-VAL-10 | Trended history ceiling required when trended enabled | UA-01–UA-03 | Bound historical exposure |
| PC-VAL-11 | Only one open Draft per product | UA-11 | Avoid competing in-progress definitions |
| PC-VAL-12 | Only Draft is editable; only Pending is decidable | UA-02, UA-04, UA-05 | Preserve immutability of published/decided items |
...
| View lineage, contract, audit | Yes | Yes | Yes | Limited |
...
| EDGE-5 | Deprecation submitted without a valid wind-down window | Blocked (PC-VAL-06); consumers must receive a valid migration period. |
...
| EDGE-12 | Packet contributes no fields to the contract | Packet still appears in lineage; its field inspection shows an empty contract for that packet. |
...
| OQ-06 | Is enquiry simulation **mandatory** before activation, as targeted by the parent epic KPI? | Data-quality gating before go-live (PC-FR-901). |
...
| AC-09 | Product detail exposes Overview, Data Contract, Lineage (submitters → packets → product → consumers with per-packet field inspection), Subscribers, Version history, and Audit trail. |
| AC-10 | A product configuration can be handed off to Enquiry Simulation without creating a real consumer enquiry footprint. |
